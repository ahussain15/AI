...
def lin_con(board):
    ...
    for k in range(size):
        ...
        for i in range(size-1):
            #Check all tiles that come after the curret
...
def astar_nh(state):
    closed = set()
    row_col(state) #Call to setup before 1st call to counter
    fringe = [(lin_con(state)+txd(state), state, 0)]
    heapq.heapify(fringe)
    while len(fringe) != 0:
        v = heapq.heappop(fringe)
        if v[1] == find_goal(state):
        ...
        if v[1] not in closed:
            closed.add(v[1])
            for c in get_children(v[1]):
                heapq.heappush(fringe, (lin_con(c)+v[2]+1+txd(c), c, v[2]+1))
    return None
...
#         a = astar(line.rstrip())
#         end = time.perf_counter()
#         print("Txd A*: %s moves in %s seconds" % (a, (end-start)))
#         start = time.perf_counter()
#         b = astar_nh(line.rstrip())
#         end = time.perf_counter()
#         print("LinCon A*: %s moves in %s seconds" % (b, (end-start)))
#         print()